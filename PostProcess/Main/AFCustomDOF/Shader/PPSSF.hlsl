   	...
   	float3 sunflare(VaryingsSF input) {
		...
		float2 sunPos = input.sunPos;
			...
			float2 depthSunPos = sunPos;
   			float depth  = GET_SCENE_DEPTH_01(depthSunPos);
		...
		#if SF_USE_GHOSTS // defined(UNITY_SINGLE_PASS_STEREO) && !defined(UNITY_STEREO_INSTANCING_ENABLED) && !defined(UNITY_STEREO_MULTIVIEW_ENABLED)
